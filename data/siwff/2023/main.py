...
def get_schedule(day):
    schedule_url = f'{BASE_URL}/schedule_view.asp?QueryStep=1&QueryDate=2023-08-{day}'
...
def parse_detail(title, text):
    ...
    try:
        ...
        synopsis_desc = f"""<p><h5><strong>SYNOPSIS</strong></h5>{synopsis}</p>"""

        pg_note = soup.find('div', {'class': 'pg_note'}).find('div', {'class': 'desc'}) if soup.find('div', {'class': 'pg_note'}) is not None else None
        pg_note_text = '<br/>'.join([get_text(d) for d in pg_note.find_all('p')]) if pg_note is not None and len(pg_note.find_all(
            'p')) > 0 else get_text(pg_note)
        pg_note_desc = f"""<p><h5><strong>PROGRAM NOTE</strong></h5>{pg_note_text}</p>""" if pg_note_text else ""

        desc = f"""{synopsis_desc}{pg_note_desc}"""
...
def _parse_film_info(title, film_info):
    ...
    keywords = [keyword.strip() for keyword in get_text(
        film_info.find('p', {'class': 'keyword'})).split('/') if keyword.strip()]
    ...
    for i in range(24, 31):
        ...
        with open(f'{day}.json', 'w', encoding='UTF-8') as f:
            ...
            json_schedule = {'name': 'siwff',
                             'year': 2023,
                             'date': day,
                             'dateStr': f'2023.08.{i} {DATE_DAY[i]}',
