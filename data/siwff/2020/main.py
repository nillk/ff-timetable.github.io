...
def get_schedule(day):
    schedule_url = f'{BASE_URL}/schedule_view.asp?QueryStep=1&QueryDate=2020-09-{day}'
...
def _parse_theater_name(element):
    return get_text(element.find('span', {'class': 'sch_li_tit_sub'}))
...
def parse_detail(title, text):
    ...
    try:
        ...
        pg_note = soup.find('div', {'id': 'pg_note'})
        desc = '\n'.join([get_text(d) for d in pg_note.find_all('p')]) if len(pg_note.find_all(
...
def _parse_film_info(title, film_info):
    ...
    if title == '풍겁':
        genre.insert(0, get_text(info_list[5]))
    ...
    if title == '여성 영화인 지원 프로젝트: 코로나 시대, 서로를 보다':
        return {
            'productionCountry': get_text(info_list[0]).split(', '),
            'yearOfProduction': get_text(info_list[1]),
            'length': get_text(info_list[2]),
            'format': get_text(info_list[4]),
            'color': get_text(info_list[5]),
            'genre': genre
        }
...
def _parse_directors(film_credit):
    ...
    directors_eng = get_text(film_credit.find(
        'div', {'class': 'dir_name'}).find('p', {'class': 'mt10'})).split(', ')
    ...
    for i in range(11, 16):
        ...
        with open(f'{day}.json', 'w', encoding='UTF-8') as f:
            ...
            json_schedule = {'name': 'siwff',
                             'year': 2020,
                             'date': day,
                             'dateStr': f'2020.09.{i} {DATE_DAY[i]}',
